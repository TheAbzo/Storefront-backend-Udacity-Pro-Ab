...
export class Orders {
    ...
    async index(): Promise<Order[]> {
        try {
            ...
            const conn = await client.connect();
            const sql =
                'select id_order, id_product,quantity, user_id, status from products full outer join products_orders on products.id = products_orders.id_product inner join orders on products_orders.id_order = orders.id';
